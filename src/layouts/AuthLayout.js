  ...
  return (
    <div className='p-4 min-h-screen grid md:grid-cols-2 text-black'>
        ...
        <div className='flex flex-col items-center md:items-start md:h-full justify-center p-8 md:p-16 gap-4'>
            ...
            <Outlet/>   
            {!token && <p>{pageTitle === 'Sign in' ? (<>New to SpotLight? Create an account <Link className='underline text-primary' to={button?.link}>here</Link></>) : (<>Already have an account? <Link className='text-primary underline' to={button.link}>Login</Link></>)}</p>}
        </div>
        </div>
        <div className='rounded-[18px] hidden md:block relative p-4 bg-red-700' style={{
